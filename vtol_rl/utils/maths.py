...
class Quaternion:
    def __init__(self, w=None, x=None, y=None, z=None, num=1, device=torch.device("cpu")):
    ...
    def extract_yaw_only(self):
        ...
        yaw = torch.atan2(2 * (self.w * self.z + self.x * self.y),
    ...
    def extract_pitch_roll(self):
        ...
        pitch = torch.atan2(2 * (self.w * self.y + self.x * self.z),
                         1 - 2 * (self.x ** 2 + self.z ** 2))
        roll = torch.atan2(2 * (self.w * self.x - self.y * self.z),
                        1 - 2 * (self.y ** 2 + self.z ** 2))
    ...
    def world_to_head(self, vec: torch.Tensor) -> torch.Tensor:
        ...
        return (q_yaw.conjugate() * Quaternion(torch.tensor(0.0, device=vec.device), *vec) * q_yaw).imag
    ...
    def local_to_head(self, vec: torch.Tensor) -> torch.Tensor:
        ...
        q_yaw = self.extract_yaw_only()
        return (q_yaw.conjugate() * Quaternion(torch.tensor(0, device=vec.device), *v_world) * q_yaw).imag
    ...
    def xz_axis(self):
        ...
        return torch.stack([
            torch.stack([1 - 2 * (self.y.clone() * self.y.clone() + self.z.clone() * self.z.clone()),
                        2 * (self.x.clone() * self.y.clone() - self.z.clone() * self.w.clone()),
                        2 * (self.x.clone() * self.z.clone() + self.y.clone() * self.w.clone())]),
            torch.stack([2 * (self.x.clone() * self.z.clone() + self.y.clone() * self.w.clone()),
    ...
    def toEuler(self, order="zyx"):
        if order == "zyx":
            roll = torch.atan2(2 * (self.w * self.x + self.y * self.z), 1 - 2 * (self.x.pow(2) + self.y.pow(2)))
            pitch = torch.asin(2 * (self.w * self.y - self.z * self.x))
            yaw = torch.atan2(2 * (self.w * self.z + self.x * self.y), 1 - 2 * (self.y.pow(2) + self.z.pow(2)))
            return torch.stack([roll, pitch, yaw])  # roll pitch yaw
        elif order == "xyz":
            roll = torch.atleast_1d(torch.atan2(2 * (self.w * self.y - self.x * self.z), 1 - 2 * (self.x.pow(2) + self.y.pow(2))))
            pitch = torch.atleast_1d(torch.asin(2 * (self.w * self.z - self.y * self.x)))
            yaw = torch.atleast_1d(torch.atan2(2 * (self.w * self.x + self.y * self.z), 1 - 2 * (self.x.pow(2) + self.z.pow(2))))
            return torch.stack([roll, pitch, yaw])  # roll pitch yaw

    @staticmethod
    def from_euler(roll, pitch, yaw, order="zyx"):
        roll, pitch, yaw = torch.as_tensor(roll), torch.as_tensor(pitch), torch.as_tensor(yaw)
        if order == "zyx":
...
class Integrator:
    ...
    @staticmethod
    def _get_derivatives(vel: torch.tensor,
                         ori: torch.tensor,
                         acc: torch.tensor,
...
def cross(a: torch.Tensor, b: torch.Tensor):
    res = torch.stack([a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2], a[0] * b[1] - a[1] * b[0]]) + 0
    return res
