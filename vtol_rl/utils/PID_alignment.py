...
class bag_plot:
    ...
    def bag_parser(self):
        for topic, msg, t in self.bag.read_messages(self.topics):
            if topic == "/bfctrl/cmd":
                ctbr_x, ctbr_y, ctbr_z, thrust = (
                    msg.angularVel.x,
                    msg.angularVel.y,
                    msg.angularVel.z,
                    msg.thrust,
                )
                self.actions_cmd.append(
                    np.array([thrust, ctbr_x, ctbr_y, ctbr_z], dtype=np.float32)
                )  # 假设动作数据存储在 msg.data 中
                self.actions_time.append(t.to_sec())

            if topic == "/mavros/setpoint_raw/attitude":
                thrust, anglevel_x, anglevel_y, anglevel_z = (
                    msg.thrust,
                    msg.body_rate.x,
                    msg.body_rate.y,
                    msg.body_rate.z,
                )
                self.action_drone.append(
                    np.array(
                        [thrust, anglevel_x, anglevel_y, anglevel_z], dtype=np.float32
                    )
                )
            ...
            if topic == "/mavros/imu/data":
                ...
                self.imu_angle_v.append(
                    np.array([angle_x, angle_y, angle_z], dtype=np.float32)
                )
    ...
    def simulator_actions(self):
        obs = self.env.reset()
        self.action_role = self.normalize(
            th.as_tensor(self.act_f(self.env.envs.dynamics.t))
        )
        self.sim_t = []
    ...
    def plot(self):
        ...
        axs.plot(
            self.sim_t, self.action_all.numpy()[:, 1], label="cmd_sim", color=colors[3]
        )
        axs.plot(
            self.imu_time, self.imu_angle_v[:, 0], label="anglex_real", color=colors[2]
        )
        axs.plot(
            self.sim_t, self.anglevel.numpy()[:, 0], label="anglex_sim", color=colors[1]
        )
        ...
        axsy.plot(
            self.sim_t, self.action_all.numpy()[:, 2], label="cmd_sim", color=colors[3]
        )
        axsy.plot(
            self.imu_time, self.imu_angle_v[:, 1], label="anglex_real", color=colors[2]
        )
        axsy.plot(
            self.sim_t, self.anglevel.numpy()[:, 1], label="anglex_sim", color=colors[1]
        )
        ...
        axsz.plot(
            self.sim_t, self.action_all.numpy()[:, 3], label="cmd_sim", color=colors[3]
        )
        axsz.plot(
            self.imu_time, self.imu_angle_v[:, 2], label="anglex_real", color=colors[2]
        )
        axsz.plot(
            self.sim_t, self.anglevel.numpy()[:, 2], label="anglex_sim", color=colors[1]
        )
