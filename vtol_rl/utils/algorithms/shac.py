...
from .common import SimpleRolloutBuffer
...
class TemporalDifferBase:
    ...
    def _build(self):
        ...
        self.actor_batch_norm_stats = get_parameters_by_name(
            self.policy.actor, ["running_"]
        )
        self.critic_batch_norm_stats = get_parameters_by_name(
            self.policy.critic, ["running_"]
        )
        self.critic_batch_norm_stats_target = get_parameters_by_name(
            self.policy.critic_target, ["running_"]
        )

    def _create_save_path(self):
        index = 1
        path = (
            f"{self.save_path}/{self.name}_{self.comment}_{index}"
    ...
    def _create_logger(
        ...
        configured_logger = logger.configure(self.policy_save_path, format_strings)
    ...
    def _create_policy(
        ...
        policy = policy_class(
            self.observation_space, self.action_space, self.lr_schedule, **policy_kwargs
        )
            ...
            with tqdm(total=total_timesteps) as pbar:
                while current_step < total_timesteps:
                    self._update_current_progress_remaining(
                        num_timesteps=current_step, total_timesteps=total_timesteps
                    )
                    ...
                    actor_loss, critic_loss = (
                        0.0,
                        0.0,
                    )  # th.tensor(0, device=self.device), th.tensor(0, device=self.device)

                    discount_factor = th.ones(
                        (self.num_envs,), dtype=th.float32, device=self.device
                    )
                    episode_done = th.zeros(
                        (self.num_envs,), device=self.device, dtype=th.bool
                    )
                    inner_step = 0
                    for inner_step in range(self.H):
                        # dream a horizon of experience
                        obs = self.env.get_observation()
                        pre_obs = obs.clone().detach()
                        # iteration
                        ...
                        next_values, _ = th.cat(
                            self.policy.critic_target(
                                obs.detach(), next_actions.detach()
                            ),
                            dim=1,
                        ).min(dim=1)
                        ...
                        done_but_not_episode_end = (
                            (done) | (inner_step == self.H - 1)
                        ) & ~episode_done
                    ...
                    for i in range(self.gradient_steps):
                        values, _ = th.cat(
                            self.policy.critic(
                                self.rollout_buffer.obs, self.rollout_buffer.action
                            ),
                            dim=1,
                        ).min(dim=1)
                        ...
                        th.nn.utils.clip_grad_norm_(
                            self.policy.critic.parameters(), 0.5
                        )
                        self.policy.critic.optimizer.step()

                        polyak_update(
                            params=self.policy.critic.parameters(),
                            target_params=self.policy.critic_target.parameters(),
                            tau=self.tau,
                        )
                        polyak_update(
                    ...
                    if pbar.n - previous_step >= self._dump_step:
                        with th.no_grad():
                            ...
                            while True:
                                ...
                                obs, reward, done, info = self.eval_env.step(
                                    clipped_actions, is_test=True
                                )
                                for index in reversed(eval_info_id_list):
                                    if done[index]:
                                        eval_info_id_list.remove(index)
                                        eq_rewards_buffer.append(
                                            info[index]["episode"]["r"]
                                        )
                                        eq_len_buffer.append(
                                            info[index]["episode"]["l"]
                                        )
                                        eq_success_buffer.append(
                                            info[index]["is_success"]
                                        )
                    ...
                    if (
                        pbar.n - previous_step >= self._dump_step
                        and len(eq_rewards_buffer) > 0
                    ):
                        self._logger.record(
                            "time/fps",
                        ...
                        self._logger.record(
                            "rollout/ep_rew_mean",
                            sum(eq_rewards_buffer) / len(eq_rewards_buffer),
                        )
                        self._logger.record(
                            "rollout/ep_len_mean",
                            sum(eq_len_buffer) / len(eq_len_buffer),
                        )
                        self._logger.record(
                            "rollout/success_rate",
                            sum(eq_success_buffer) / len(eq_success_buffer),
                        )
                        self._logger.record("train/actor_loss", actor_loss.item())
                        self._logger.record(
                            "train/critic_loss",
                            critic_loss.item()
                            if isinstance(critic_loss, th.Tensor)
                            else critic_loss,
                        )
