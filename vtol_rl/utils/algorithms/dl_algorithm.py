...
from typing import Union, Dict, Optional, Type, Any, ClassVar
...
class ApgBase:
    policy_aliases: ClassVar[Dict[str, Type[BaseApgPolicy]]] = {
    ...
    def _create_save_path(self):
        index = 1
        path = (
            f"{self.save_path}/{self.name}_{self.commit}_{index}"
                        ...
                        for indice in range(self.num_envs):
                            if self.envs.done[indice]:
                                rewards_buffer.append(
                                    self.envs.info[indice]["episode"]["r"]
                                )
                                len_buffer.append(
                                    self.envs.info[indice]["episode"]["l"]
                                )
                                success_buffer.append(
                                    self.envs.info[indice]["is_success"]
                                )

                        if (
                            pbar.n - previous_step >= self._dump_step
                            and len(rewards_buffer) > 0
                        ):
                            self._logger.record(
                                "time/fps",
                            ...
                            self._logger.record(
                                "rollout/ep_rew_mean",
                                sum(rewards_buffer) / len(rewards_buffer),
                            )
                            self._logger.record(
                                "rollout/ep_len_mean", sum(len_buffer) / len(len_buffer)
                            )
                            self._logger.record(
                                "rollout/success_rate",
                                sum(success_buffer) / len(success_buffer),
                            )
                            self._logger.record(
                                "rollout/ep_horizon_mean",
                                sum(len_horizon_buffer) / len(len_horizon_buffer)
                                if len(len_horizon_buffer) > 0
                                else None,
                            )
