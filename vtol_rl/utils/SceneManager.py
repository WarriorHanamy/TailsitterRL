...
from .common import habitat_to_std, std_to_habitat
...
# BGR
ColorSet5 = [
    mn.Color4(45.0 / 255, 45.0 / 255, 45.0 / 255, 0.1),
    mn.Color4(255.0 / 255, 96.0 / 255, 0.0 / 255, copacity),
    mn.Color4(255.0 / 255, 165.0 / 255, 89.0 / 255, copacity),
    mn.Color4(255.0 / 255, 230.0 / 255, 199.0 / 255, copacity),
]
ColorSet = [
    mn.Color4(69.0 / 255, 69.0 / 255, 69.0 / 255, copacity),  # default color
    mn.Color4(0.0 / 255, 96.0 / 255, 255.0 / 255, copacity),  # highlight color1
    mn.Color4(89.0 / 255, 165.0 / 255, 255.0 / 255, copacity),  # highlight color2
    mn.Color4(199.0 / 255, 230.0 / 255, 255.0 / 255, copacity),  # highlight color3
]

ColorSet2 = [
    mn.Color4(234.0 / 255, 223.0 / 255, 180.0 / 255, copacity),  # default color
    mn.Color4(155.0 / 255, 176.0 / 255, 193.0 / 255, copacity),  # highlight color1
    mn.Color4(81.0 / 255, 130.0 / 255, 246.0 / 255, copacity),  # highlight color2
    mn.Color4(246.0 / 255, 153.0 / 255, 92.0 / 255, copacity),  # highlight color3
]
...
ColorSet6 = [
    mn.Color4(0.0 / 255, 128.0 / 255, 157.0 / 255, 1.0),  # default color
    mn.Color4(252.0 / 255, 236.0 / 255, 221.0 / 255, copacity),  # highlight color1
    mn.Color4(255.0 / 255, 117.0 / 255, 1.0 / 255, copacity),  # highlight color2
    mn.Color4(242.0 / 255, 162.0 / 255, 109.0 / 255, copacity),  # highlight color3
]  # rgb

ColorSet4 = [
    mn.Color4(144.0 / 255, 156.0 / 255, 33.0 / 255, 0.5),  # default color
    mn.Color4(36.0 / 255, 184.0 / 255, 233.0 / 255, copacity),  # highlight color1
    mn.Color4(34.0 / 255, 147.0 / 255, 238.0 / 255, copacity),  # highlight color2
    mn.Color4(49.0 / 255, 63.0 / 255, 216.0 / 255, copacity),  # highlight color3
]  # bgr
...
class SceneManager(ABC):
    def __init__(
        ...
        _dataLoader = SimpleDataLoader(
            ChildrenPathDataset(self.root_path, type=scene_type, semantic=semantic),
            batch_size=num_scene,
            shuffle=True,
        )
        ...
        if obj_settings:
            ...
            _objLoader = SimpleDataLoader(
                ChildrenPathDataset(obj_path, type="obj", semantic=False),
                batch_size=num_scene,
                shuffle=True,
            )
            self._obj_loader = _objLoader
        self.dynamic_object_position = [
            [None] for _ in range(num_agent_per_scene * num_scene)
        ]
        self.dynamic_object_velocity = [
            [None] for _ in range(num_agent_per_scene * num_scene)
        ]
        self.dynamic_object_acceleration = [
            [None] for _ in range(num_agent_per_scene * num_scene)
        ]
        ...
        self._obj_mgrs: habitat_sim.physics.RigidObjectManager = [
        ...
        if self.render_settings is not None:
            self.render_settings["object_path"] = self.render_settings.get(
                "object_path", self._drone_path
            )
            ...
            self.render_settings["velocity"] = self.render_settings.get(
                "velocity", False
            )
            ...
            self.render_settings["collision"] = self.render_settings.get(
                "collision", False
            )

            if self.render_settings["position"] is not None:
                self.render_settings["position"] = th.as_tensor(
                    self.render_settings["position"], dtype=th.float32
                )

            self._render_camera = [None for _ in range(num_scene)]
            self._line_mgrs: habitat_sim.gfx.DebugLineRender = [
                None for _ in range(num_scene)
            ]
            self._drones = [
                [None for _ in range(num_agent_per_scene)] for _ in range(num_scene)
    ...
    def _get_datasets_info(self, path):
        ...
        self._drone_path = [
            root_addr
            + "datasets/visfly-beta/configs/agents/DJI_Mavic_"
            + c
            + ".object_config.json"
        ...
        if "hm3d" in parts[index].lower():
            ...
            self._datasets_path = (
                root_addr + "datasets/visfly-beta/visfly-beta.scene_dataset_config.json"
            )
            ...
            self._datasets_path = (
                root_addr
                + "datasets/spy_datasets/spy_datasets.scene_dataset_config.json"
            )
        elif "hssd" in parts[index].lower():
            self.datasets_name = "hssd-hab"
            self._datasets_path = (
                root_addr + "datasets/hssd-hab/hssd-hab.scene_dataset_config.json"
            )
    ...
    def set_pose(self, position, rotation):
        ...
        for scene_id in range(self.num_scene):
            for agent_id in range(self.num_agent_per_scene):
                ...
                self.trajectory[scene_id][agent_id].insert(
                    0, np.hstack([hab_pos[drone_id], hab_ori[drone_id]])
                )
                drone_id += 1
                if self.is_multi_drone or self.render_settings:
                    self._drones[scene_id][
                        agent_id
    ...
    def _update_collision_infos(
        self, indices: Optional[List] = None, sensitive_radius: float = None
    ):
        """_summary_test
        ...
        sensitive_radius = (
            self.sensitive_radius if sensitive_radius is None else sensitive_radius
        )
        ...
        for indice in indices:
            ...
            (
                ...
                self._is_out_bounds[scene_id][agent_id],
            ) = col_record.hit_pos, col_record.is_out_bound
    ...
    def render(
        ...
        def draw_axes(lr, translation, axis_len=1.0):
            ...
            if ENU_coordnates_axes:
                lr.draw_transformed_line(
                    translation, mn.Vector3(0, 0, -axis_len), ColorSet[1]
                )
                lr.draw_transformed_line(
                    translation, mn.Vector3(-axis_len / 2, 0, 0), ColorSet[2]
                )
                lr.draw_transformed_line(
                    translation, mn.Vector3(0, axis_len / 2, 0), ColorSet[3]
                )
            else:
                # draw axes with x+ = red, y+ = green, z+ = blue
                # debug: it should be rgb but bgr presented, i don't know why
                lr.draw_transformed_line(
                    translation, mn.Vector3(axis_len, 0, 0), ColorSet[1]
                )
                lr.draw_transformed_line(
                    translation, mn.Vector3(0, axis_len / 2, 0), ColorSet[2]
                )
                lr.draw_transformed_line(
                    translation, mn.Vector3(0, 0, axis_len / 2), ColorSet[3]
                )
        ...
        if curves is not None:
            for curve in curves:
                ...
                self._line_mgrs[0].draw_path_with_endpoint_circles(curve, 0.1, white)
            ...
            for scene_id in range(self.num_scene):
                for agent_id in range(self.num_agent_per_scene):
                    if self._collision_point[scene_id][agent_id] is not None:
                        ...
                        p2 = np.array(
                            self.agents[scene_id][agent_id].scene_node.translation
                        )
                        self._line_mgrs[scene_id].draw_transformed_line(
                            p2,
                            p1,
                            color=color_consequence(
                                factor=np.linalg.norm(p2 - p1) / 2,
                                color1=ColorSet6[2],
                                color2=ColorSet6[0],
                            ),
                        )
                ...
                for scene_id in range(self.num_scene):
                    if self.render_settings["position"] is None:
                        ...
                        scene_center = std_to_habitat(
                            self.render_settings["position"], None
                        )[0][0]
    ...
    def load_scenes(self, indices=None):
        ...
        for scene_id in indices:
            ...
            if self.render_settings is not None or self.is_multi_drone:
                ...
                self._line_mgrs[scene_id] = self.scenes[
                    scene_id
                ].get_debug_line_render()
                self._line_mgrs[scene_id].set_line_width(
                    self.render_settings["line_width"]
                )
                # create objects in each scene
                for agent_id in range(self.num_agent_per_scene):
                    self._drones[scene_id][agent_id] = self._obj_mgrs[
                        scene_id
            ...
            if self.obj_settings:
                ...
                print(debug_p)
                self._obj_ctrls[scene_id] = ObjectManager(
    ...
    def _load_scene(self, scene_path) -> habitat_sim.Simulator:
        ...
        sim_cfg.scene_dataset_config_file = self._datasets_path
    ...
    def reset(
        self, std_positions: Tensor, std_orientations: Tensor
    ) -> Tuple[List, List]:
        """
    ...
    def reset_agents(
        self,
        std_positions: Tensor,
        std_orientations: Tensor,
    ...
    def _load_sensor(self) -> List[habitat_sim.sensor.SensorSpec]:
        ...
        for i, sensor_cfg in enumerate(self.sensor_settings):
            ...
            sensor_spec.orientation = mn.Vector3(
                sensor_cfg.get("orientation", [0.0, 0, 0])
            )
            ...
            if not isinstance(
                sensor_type, habitat_sim._ext.habitat_sim_bindings.SensorType
            ):
                continue
    ...
    def _update_dynamics(self):
        self.dynamic_object_position = [
            ...
            for _ in range(self.num_agent_per_scene)
        ]
        self.dynamic_object_velocity = [
            obj_ctrl.velocity
            for obj_ctrl in self._obj_ctrls
