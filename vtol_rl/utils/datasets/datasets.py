...
scene_example = {
    ...
    "user_custom": {"bound": [[1, 1, 1], [10, 10, 10]]},
}
...
class SceneGenerator:
    def __init__(
        self, path: str, num: int, name: str, setting: SceneGeneratorSetting
    ) -> None:
        self.path = path
        self.num = num
        self.name = name
        ...
        self.object_root_path = os.path.join(
            path, f"configs/{setting.object_set}"
        )  # debug
    ...
    def _create_scene_json(self):
        ...
        for id in range(self.num):
            ...
            # update stage range
            if (
                scene_template_cache is None
                or scene_json["stage_instance"]["template_name"] != scene_template_cache
            ):
                scene_template_cache = scene_json["stage_instance"]["template_name"]
                scene_json["user_custom"]["bound"] = self._get_stage_bound(
            ...
            scene_json["object_instances"] = self._create_objects(
                self.setting.object_dense,
    ...
    def _get_stage_bound(self, stage_name, debug_stage=""):
        ...
        if "garage" in stage_name:
            return [[0, -7, 0], [18, 7, 5.0]]
        elif "box10_wall" in stage_name:
            return [[0, -10, 0], [20, 10, 8.0]]
        elif "box15_wall" in stage_name:
            return [[0, -15, 0], [30, 15, 8.0]]
        ...
        habitat_sim_cfg.scene_dataset_config_file = (
            "datasets/visfly-beta/visfly-beta.scene_dataset_config.json"
        )
        habitat_sim_cfg.enable_physics = False
        agent_cfg = habitat_sim.agent.AgentConfiguration()
        sim = habitat_sim.Simulator(
            habitat_sim.Configuration(habitat_sim_cfg, [agent_cfg])
        )
    ...
    def _create_objects(
        ...
        positions = [
            np.random.uniform(bounds[0, :], bounds[1, :]) for _ in range(num_points)
        ]
        ...
        max_scale = 2
        sizes = [
            1 + scale_randomness * np.random.uniform(0, max_scale - 1)
            for _ in range(num_points)
        ]
...
class ChildrenPathDataset(Dataset):
    def __init__(self, root_path, type="glb", semantic=False):
        ...
        if len(self.paths) == 0:
            if type == "obj":
                root_path = (
                    ...
                    + f"configs/obj/{root_path}"
                )
                self.paths = self._load_scene_path(
                    semantic=semantic, root_path=root_path
                )
        if len(self.paths) == 0:
        ...
        if len(self.paths) == 0:
            ...
            print(
                f"Warning: No files found in the VisFly object path, trying to find in local path: \n{folder_path}."
            )
...
def parsers():
    ...
    parser.add_argument(
        "--scene", "-s", type=str, default="garage", help="obstacle density"
    )
