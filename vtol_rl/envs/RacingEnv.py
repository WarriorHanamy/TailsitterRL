...
class RacingEnv(DroneGymEnvsBase):
    def __init__(
        ...
        self.observation_space["gate"] = spaces.Box(
            low=0, high=len(self.targets), shape=(1,), dtype=np.int32
        )
    ...
    def get_success(self) -> th.Tensor:
        ...
        self._is_pass_next = (self.position - self.targets[_next_target_i_clamp]).norm(
            dim=1
        ) <= self.success_radius
    ...
    def reset_agent_by_id(self, agent_indices=None, state=None, reset_obs=None):
        agent_indices = (
            th.arange(self.num_envs) if agent_indices is None else agent_indices
        )
        ...
        self._past_targets_num[agent_indices] = th.zeros(
            (len(agent_indices),), dtype=th.int
        )
        self._is_pass_next[agent_indices] = th.zeros(
            (len(agent_indices),), dtype=th.bool
        )
    ...
    def get_reward(self) -> th.Tensor:
        if not is_pos_reward:
            ...
            approaching_v = (dis_v_product / (dis + 1e-6)).clamp_max(15.0)
...
class RacingEnv2(RacingEnv):
    ...
    def get_observation(
        ...
        relative_pos = (next_targets - self.position.unsqueeze(1)).reshape(
            self.num_envs, -1
        )
        state = th.hstack(
            [
