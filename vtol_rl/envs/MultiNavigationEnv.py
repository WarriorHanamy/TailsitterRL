...
class MultiNavigationEnv(MultiDroneGymEnvBase):
    def __init__(
        ...
        self.target = (
            th.vstack(
                [
                    th.tensor([[13, -2.0, 1.5], [13, -0.0, 1.5], [13, 2.0, 1.5]])
        ...
        self.observation_space["swarm"] = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(
                self.num_agent_per_scene - 1,
        ...
        if self.is_find_path:
            raise ValueError("Path finding via habitat_sim is no longer supported.")
        ...
        for scene_index in range(self.num_scene):
            for agent_index in range(self.num_agent_per_scene):
                ...
                sequence = th.cat(
                    [
                        th.arange(
                            scene_index * self.num_agent_per_scene,
                            agent_i,
                            dtype=th.uint8,
                        ),
                        th.arange(
                            agent_i + 1,
                            (scene_index + 1) * self.num_agent_per_scene,
                            dtype=th.uint8,
                        ),
                    ]
                )
    ...
    def _synthesize_depth(self) -> th.Tensor:
        distance = (self.target - self.position).norm(dim=1, keepdim=True)
    ...
    def get_reward(self) -> th.Tensor:
        ...
        reward = (
            ...
            * base_r
            * (0.5 + 0.5 / (1 + 1 * self.velocity.norm(dim=1)))
        )
